...
router.post(
  "/login",
  [
    check("dni", "El CUIL de usuario no cumple con los requisitos")
      .not()
      .isEmpty()
      .isInt()
      .isLength({ min: 11, max: 11 }),
    check("password", "La contraseña no cumple con los requisitos").isLength({
      min: 4,
      max: 30,
    }),
    validateFields,
...
router.delete(
  "/",
  [auth, verifyRole, check("id").not().isEmpty(), validateFields],
  borrarUsuario
);
